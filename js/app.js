...
(async function () {
  ...
  function mountExport(host) {
    host.innerHTML = `
      ...
      <div class="panel" style="margin-bottom:1rem">
        <h2>Export</h2>
        <p class="hint"><b>nfrs.yaml</b> machine-readable · <b>nfrs.md</b> governance spec · <b>ADRs</b> trade-off decisions · <b>Jira CSV</b> (Epic + a Story per NFR, bulk-importable) · <b>Epic + Stories</b> markdown · <b>PRD</b> — all built from the in-scope NFRs for tracking in Jira/your backlog.</p>
        <div class="row"><div class="seg" role="group" aria-label="Export format">
          <button class="active" data-tab="yaml" aria-pressed="true">nfrs.yaml</button><button data-tab="md" aria-pressed="false">nfrs.md</button><button data-tab="adr" aria-pressed="false">ADRs</button><button data-tab="csv" aria-pressed="false">Jira CSV</button><button data-tab="epic" aria-pressed="false">Epic + Stories</button><button data-tab="prd" aria-pressed="false">PRD</button>
    ...
    // ---- Jira-trackable exports (Epic + Stories CSV, Epic+Stories md, PRD) ----
    const PRIO={critical:"Highest",high:"High",medium:"Medium",low:"Low"};
    const csvEsc=s=>`"${String(s==null?"":s).replace(/"/g,'""')}"`;
    const story=(n)=>{const q=n.scenario||{};return {
      summary:`${n.name} meets its target`,
      desc:`*NFR:* ${n.name} (${n.iso})\n*Business impact if not met:* ${n.businessImpact||"—"}\n*Scenario:* [${q.source}] ${q.stimulus} → ${q.response} (${q.environment})\n*Tactics:* ${(n.tactics||[]).join("; ")}`,
      ac:`Given ${q.environment||"normal operation"}, when ${q.stimulus||"exercised"}, then ${q.response||"the quality holds"}.\nSLO: ${q.measure||"—"}\nVerified by: ${n.fitnessFunction||"—"}`,
      prio:PRIO[n.severity]||"Medium",
      labels:[n.category,n.mandatory?"mandatory":"",...(n.regs||[])].filter(Boolean).join(";")
    };};
    function toCsv(){const {ctx,rk,regs}=gather();const epic=`NFRs — ${ctx.domain} (${ctx.region})`;
      const ctxLine=Object.entries(ctx).map(([k,v])=>`${k}=${v}`).join(", ");
      const head=["Issue Type","Summary","Description","Acceptance Criteria","Priority","Labels","Epic Name","Epic Link"];
      const rows=[head.map(csvEsc).join(",")];
      rows.push(["Epic",epic,`Non-functional requirements for this system.\nContext: ${ctxLine}\nRegulations in scope: ${regs.map(r=>r.name).join(", ")||"none"}.\nGenerated by NFR Advisor.`,"","High","nfr;architecture",epic,""].map(csvEsc).join(","));
      rk.forEach(n=>{const s=story(n);rows.push(["Story",s.summary,s.desc,s.ac,s.prio,s.labels,"",epic].map(csvEsc).join(","));});
      return rows.join("\n");}
    function toEpicMd(){const {ctx,rk,regs}=gather();
      let m=`# Epic: Non-Functional Requirements — ${ctx.domain} (${ctx.region})\n\n_Generated by NFR Advisor — ${rk.length} stories from the in-scope NFRs._\n\n`;
      m+=`**Context:** ${Object.entries(ctx).map(([k,v])=>`${k}=${v}`).join(" · ")}\n\n`;
      if(regs.length)m+=`**Compliance in scope:** ${regs.map(r=>r.name).join(", ")}\n\n`;
      m+=`**Definition of done:** every story meets its SLO and is verified by its fitness function; all trade-offs resolved.\n\n---\n\n`;
      rk.forEach((n,i)=>{const q=n.scenario||{};const s=story(n);
        m+=`## Story ${i+1}: ${n.name}${n.mandatory?" — MANDATORY":""}\n\n**Priority:** ${s.prio} · **Labels:** ${[n.category,...(n.regs||[])].join(", ")}\n\n`;
        m+=`> As the team building this system, we want **${n.name.toLowerCase()}** to meet its target, so that ${(n.businessImpact||"the system stays trustworthy").replace(/\.$/,"")}.\n\n`;
        m+=`**Acceptance criteria**\n- Given ${q.environment||"normal operation"}, when ${q.stimulus||"exercised"}, then ${q.response||"the quality holds"}.\n- **SLO:** ${q.measure||"—"}\n- **Verify:** ${n.fitnessFunction||"—"}\n`;
        if((n.tactics||[]).length)m+=`- **Tactics:** ${n.tactics.join("; ")}\n`;
        m+=`\n`;});
      return m;}
    function toPrd(){const {ctx,rk,conflicts,priorities,regs}=gather();
      const high=rk.filter(n=>n.tier==="high"),med=rk.filter(n=>n.tier==="medium");
      let m=`# PRD — Non-Functional Requirements\n\n_System: ${ctx.domain} · ${ctx.region} · ${ctx.userScale} · criticality ${ctx.systemCriticality}. Generated by NFR Advisor._\n\n`;
      m+=`## 1. Overview\n\nThis document specifies the quality attributes (NFRs) the system must meet, derived from its context. ${rk.length} NFRs are in scope (${high.length} high, ${med.length} medium)${regs.length?`; ${regs.length} regulations apply`:""}.\n\n`;
      m+=`## 2. System context\n\n| Dimension | Value |\n|---|---|\n`;Object.entries(ctx).forEach(([k,v])=>m+=`| ${k} | ${v} |\n`);m+=`\n`;
      if(regs.length){m+=`## 3. Compliance obligations\n\n`;regs.forEach(r=>m+=`- **${r.name}** — ${r.control} (mandates: ${(r.drives||[]).join(", ")})\n`);m+=`\n`;}
      m+=`## 4. Requirements & success metrics\n\nEach NFR is a requirement; its SLO is the success metric.\n\n`;
      [["High importance",high],["Medium importance",med]].forEach(([t,g])=>{if(!g.length)return;m+=`### ${t}\n\n`;
        g.forEach(n=>{const q=n.scenario||{};m+=`- **${n.name}**${n.mandatory?" _(mandatory)_":""} — ${q.measure||"—"}  \n  _Why:_ ${n.businessImpact||"—"}\n`;});m+=`\n`;});
      m+=`## 5. Key trade-off decisions\n\n`;if(!conflicts.length)m+=`_None for this context._\n`;conflicts.forEach(e=>{const w=priorities[e.key];m+=`- **${e.a.name} ↔ ${e.b.name}** — ${w==="balanced"?"balanced (accept both)":(w?`prioritize ${w===e.a.id?e.a.name:e.b.name}`:"_open_")}\n`;});m+=`\n`;
      m+=`## 6. Out of scope\n\nFunctional behaviour, UI copy, and quality attributes of low relevance for this context.\n`;
      return m;}
    const GEN={ yaml:[toYaml,"nfrs.yaml","text/plain"], md:[toMd,"nfrs.md","text/plain"], adr:[toAdr,"adr-tradeoffs.md","text/plain"], csv:[toCsv,"nfr-jira-import.csv","text/csv"], epic:[toEpicMd,"nfr-epic-stories.md","text/plain"], prd:[toPrd,"nfr-prd.md","text/plain"] };
    const content=()=>(GEN[tab]||GEN.yaml)[0]();
    const filename=()=>(GEN[tab]||GEN.yaml)[1];
    const mime=()=>(GEN[tab]||GEN.yaml)[2];
    ...
    host.querySelector("#dlBtn").addEventListener("click",()=>{const blob=new Blob([content()],{type:mime()});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=filename();a.click();URL.revokeObjectURL(a.href);});
